...
def get_execution_file(reference: str) -> dict | None:
    """
    يجلب بيانات ملف تنفيذ أساسية (بدون سجلات البريد).
    """
    try:
        ...
        frappe.log_error(
            ...
            title="Execution Mail Editor Get Error",
        )
        return None
...
def search_execution_files(
    ...
    try:
        ...
        frappe.log_error(
            message=frappe.get_traceback(),
            title="Execution Mail Editor Search Error",
        )
        return []
...
def _save_batch(
    ...
    try:
        ...
        if is_post:
            ...
            # إذا كان الحفظ كمسودة والمحفظة مرحّلة، نعيدها للمسودة (حالة إضافة رد)
            doc.status = BATCH_STATUS_DRAFT

        # إعادة بناء البنود الداخلية
        doc.set("execution_items", [])
        for r in items:
            doc.append("execution_items", {
        ...
        # إذا كان ترحيلاً: إضافة/تحديث سجلات البريد في ملفات التنفيذ الحية
        if is_post:
            for item in doc.execution_items:
                ...
                if exec_file_name:
                    ...
                    existing_mail_data = frappe.db.get_value(
                        "Mail Data",
                        {
                            "parent": exec_file_name,
                            "parenttype": "Execution File",
...
def save_batch_draft(
    ...
    """يحفظ محفظة بريد كمسودة (دون تعديل ملفات التنفيذ الحية)."""
...
def post_batch_archive(
    ...
    """يرحّل المحفظة البريدية نهائياً ويُضيف سجلات البريد إلى ملفات التنفيذ الحية."""
    records = json.loads(items) if items else []
    return _save_batch(
        items=records,
        is_post=True,
...
def get_files_with_mail_status(file_status="منظور", target_mail_status="استكمال الاطلاع"):
    """
    جلب ملفات التنفيذ التي أحدث سجل بريد مرسل لها حالته 'استكمال الإطلاع'

    Args:
        file_status (str): حالة ملف التنفيذ المطلوبة (افتراضي: "منظور")
        target_mail_status (str): حالة البريد المطلوبة لأحدث سجل (افتراضي: "استكمال الإطلاع")

    Returns:
        list: قائمة بملفات التنفيذ مع بيانات البريد المرتبطة بها
    """
    query = """
        SELECT
            f.name,
            ...
            f.respondent,
            f.judge,
            f.execution_assistant,
            f.current_execution_assistant,
            f.status,
            m.mail_status,
            m.upload_mail,
            m.upload_mail_date,
...
# ---------------------------------------------------------------------------
# جلب جميع الملفات المنظورة مع أحدث سجل بريد (بدون فلترة mail_status)
# ---------------------------------------------------------------------------

@frappe.whitelist()
def get_files_with_any_mail_status(file_status=None, mail_status=None):
    """
    جلب ملفات التنفيذ التي لها أحدث سجل بريد، مع فلترة اختيارية حسب حالة الملف و/أو حالة البريد

    Args:
        file_status (str, optional): حالة ملف التنفيذ للفلترة (افتراضي: None = الكل)
