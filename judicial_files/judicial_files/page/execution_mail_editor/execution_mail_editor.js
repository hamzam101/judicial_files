// =============================================================================
// execution_mail_editor.js — محرر البريد لملفات التنفيذ
// =============================================================================
// المعمارية: Data-Driven Architecture — الفصل التام بين البيانات والواجهة
// =============================================================================
// Namespace  │ المسؤولية
// -----------┼--------------------------------------------------------------
// EME.State  │ الحالة العامة (records[], mail_statuses, pagination...)
// EME.Store  │ Actions: add_row / update_row_field / delete_row / clear
// EME.Logic  │ Business Rules: قواعد العمل والتبعيات بين الحقول
// EME.Template│ HTML Template: توليد HTML الصف من كائن البيانات
// EME.UI     │ Rendering: renderGrid / renderRow / pagination / reindex
// EME.Grid   │ Grid Actions: perform_search / add_file / delete_row
// EME.Batch  │ API Layer: save_draft / load_batch / post_batch
// EME.Print  │ Printing: action_print / action_print_all
// EME.Events │ Event Delegation: مستمع أحداث موحد على مستوى الجدول
// =============================================================================
...
// =============================================================================
// EME.State — الحالة العامة لمحرر البريد
// =============================================================================
...
// =============================================================================
// EME.Store — إدارة البيانات المركزية (Actions)
// =============================================================================
EME.Store = {
    // إضافة صف جديد
    add_row: function (data_obj) {
        ...
        EME.State.records.unshift(data_obj); // الإضافة في الأعلى
        return data_obj;
    },

    // تحديث حقل معين في صف
    update_row_field: function (id, field, value) {
        var row = EME.State.records.find(function (r) { return r._id === id; });
        if (row) {
            row[field] = value;
    ...
    delete_row: function (id) {
        var idx = EME.State.records.findIndex(function (r) { return r._id === id; });
        if (idx !== -1) {
            EME.State.records.splice(idx, 1);
            return true;
        }
        return false;
    },
    ...
    clear_records: function () {
        EME.State.records = [];
        EME.State.scanned_references.clear();
        EME.State.scan_index = 0;
    },

    // جلب جميع السجلات
    get_all: function () {
        return EME.State.records;
    }
};

// =============================================================================
// EME.Logic — هندسة قواعد العمل (Business Rules)
// =============================================================================
EME.Logic = {
...
// =============================================================================
// EME.Template — نظام القوالب (Smart Template Engine)
// =============================================================================
EME.Template = {
    row: function (row) {
        ...
        var filtered_statuses = EME.State.mail_statuses.filter(function (ms) {
            if (EME.State.is_reply_mode) return ms.mail_status !== 'معلق';
        ...
        var field_disabled_attr = EME.State.is_reply_mode ? '' : (mail_disabled ? 'disabled' : '');
        // في مرحلة رفع البريد (غير reply mode)، حقول الرد تكون مقفلة دائماً
        var reply_field_disabled_attr = EME.State.is_reply_mode ? '' : 'disabled';
        ...
        var html = '<tr id="' + row._id + '" class="' + row_class + '" data-id="' + row._id + '">'
            ...
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (row.execution_assistant || '') + '" data-fieldname="execution_assistant" ' + readonly_attr + ' tabindex="-1"></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (row.current_execution_assistant || '') + '" data-fieldname="current_execution_assistant" ' + readonly_attr + ' tabindex="-1"></td>'
...
EME.UI = {
    ...
    renderRow: function (id) {
        ...
        var old_tr = $('#' + id);
        if (old_tr.length) {
            var new_html = EME.Template.row(row);
    ...
    updateMenuVisibility: function () {
        if (EME.State.is_readonly_mode || EME.State.is_reply_mode) {
    ...
    apply_pagination: function () {
        ...
        var is_all = (EME.State.records_per_page === Infinity || isNaN(EME.State.records_per_page));
        var total_pages = is_all ? 1 : Math.ceil(total_records / EME.State.records_per_page);

        if (EME.State.current_page > total_pages) EME.State.current_page = total_pages || 1;
        if (EME.State.current_page < 1) EME.State.current_page = 1;

        var start_idx = is_all ? 1 : (EME.State.current_page - 1) * EME.State.records_per_page + 1;
        var end_idx = is_all ? total_records : Math.min(EME.State.current_page * EME.State.records_per_page, total_records);
        ...
        var page_info = is_all
            ...
            : __('إجمالي') + ': <strong>' + total_records + '</strong> ' + __('سجل') + ' | ' + __('صفحة') + ' <strong>' + EME.State.current_page + '</strong> ' + __('من') + ' <strong>' + total_pages + '</strong>';
        $('#pagination-info').html(page_info);

        $('#pagination-btn-prev').prop('disabled', is_all || EME.State.current_page <= 1)
            .css({ opacity: (is_all || EME.State.current_page <= 1) ? '0.5' : '1', cursor: (is_all || EME.State.current_page <= 1) ? 'not-allowed' : 'pointer' });
        $('#pagination-btn-next').prop('disabled', is_all || EME.State.current_page >= total_pages)
            .css({ opacity: (is_all || EME.State.current_page >= total_pages) ? '0.5' : '1', cursor: (is_all || EME.State.current_page >= total_pages) ? 'not-allowed' : 'pointer' });
    ...
    reindex_rows: function () {
        ...
        EME.State.scan_index = current_row - 1;
        this.apply_pagination();
    },
            ...
            $input.datepicker({
                ...
                onSelect: function(formattedDate, date, inst) {
                    if (date) {
                        ...
                        if (id && fieldname) {
                            var row = EME.Store.update_row_field(id, fieldname, sysDate);
                            if (row) {
                                EME.Grid.mark_batch_as_dirty();
                            }
                        }
                        ...
                        var fieldname = $input.data('fieldname');
                        if (id && fieldname) {
                            var row = EME.Store.update_row_field(id, fieldname, '');
                            if (row) {
                                EME.Grid.mark_batch_as_dirty();
                            }
                        }
...
// =============================================================================
// EME.Grid — إدارة الجدول (إضافة، بحث، تعديل، حذف)
// =============================================================================

EME.Grid = {
    // --- تعليم المسودة على أنها غير محفوظة ---
    mark_batch_as_dirty: function () {
        if (EME.State.is_readonly_mode && !EME.State.is_reply_mode) return;
        $('#mail-btn-post').hide();
    ...
    perform_search: function () {
        ...
        if (EME.State.scanned_references.has(reference)) {
        ...
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.get_execution_file',
            args: { reference: reference },
            callback: function (r) {
                if (r.message) {
                    EME.Grid.add_file_to_grid(r.message, true);
                    EME.State.scanned_references.add(reference);
                    frappe.show_alert({ message: __('تم العثور على الملف وجلب بياناته.'), indicator: 'green' });
                } else {
                    EME.Grid.add_file_to_grid({
    ...
    add_mail_row_to_grid: function (file, mail_row, exists) {
        ...
        var row_obj = {
            ...
            judge: file.judge || '',
            execution_assistant: file.execution_assistant || '',
            current_execution_assistant: file.current_execution_assistant || '',
            status: file.status || '',
        ...
        if (is_new_row) {
            row_obj = EME.Logic.apply_new_row_rules(row_obj);
        } else {
        ...
        EME.Store.add_row(row_obj);
        EME.UI.renderGrid();

        EME.State.current_page = 1;
    ...
    research_row_reference: function (tr) {
        ...
        var duplicate = EME.State.records.some(function (r) {
        ...
        frappe.call({
            ...
            callback: function (r) {
                if (r.message) {
                    var file = r.message;
                    if (old_ref) EME.State.scanned_references.delete(old_ref);
                    EME.State.scanned_references.add(new_ref);
                    ...
                    row.execution_assistant = file.execution_assistant;
                    row.current_execution_assistant = file.current_execution_assistant;
                    row.status = file.status;
                    row.is_missing = false;

                    EME.Logic.apply_new_row_rules(row);
                    EME.UI.renderGrid();
                    EME.Grid.mark_batch_as_dirty();
    ...
    trigger_row_search_lookup: function (tr) {
        ...
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.search_execution_files',
            args: { file_number: file_number, year: year, petitioner: petitioner, respondent: respondent },
            callback: function (r) {
                if (r.message && r.message.length > 0) {
                    if (r.message.length === 1) {
                        EME.Grid.populate_tr_with_file(tr, r.message[0]);
                    } else {
                        EME.Grid.show_selection_dialog(tr, r.message);
                    }
                } else {
    ...
    populate_tr_with_file: function (tr, file) {
        ...
        if (row.reference) EME.State.scanned_references.delete(row.reference);
        EME.State.scanned_references.add(file.reference);
    ...
    show_selection_dialog: function (tr, files) {
        ...
        $(document).off('click', '#dialog-lookup-table tbody tr').on('click', '#dialog-lookup-table tbody tr', function () {
            ...
            if (files[idx]) EME.Grid.populate_tr_with_file(tr, files[idx]);
            dialog.hide();
        });
    },
};
...
// =============================================================================
// EME.Batch — إدارة المحفظة البريدية (حفظ، تحميل، ترحيل)
// =============================================================================
...
EME.Batch = {
    ...
    collect_current_grid_items: function () {
        ...
        records.forEach(function (r) {
            ...
            items.push({
                ...
                execution_file: r.file_number ? r.docname || r.name || '' : '',
                file_number: r.file_number || '',
                year: r.year || '',
                petitioner: r.petitioner || '',
                respondent: r.respondent || '',
                judge: r.judge || '',
                original_assistant: r.execution_assistant || '',
                current_assistant: r.current_execution_assistant || '',
    ...
    has_reply_data: function () {
        for (var i = 0; i < EME.State.records.length; i++) {
            var r = EME.State.records[i];
    ...
    clear_grid: function () {
        ...
        EME.Store.clear_records();
        EME.State.current_page = 1;
        EME.State.active_batch_name = null;
        EME.State.active_batch_title = '';
        EME.State.active_batch_description = '';
        EME.State.is_readonly_mode = false;
        EME.State.is_reply_mode = false;
    ...
    load_batch_into_grid: function (batch_data, readonly) {
        EME.Store.clear_records();
        EME.State.is_readonly_mode = !!readonly;
        EME.State.is_reply_mode = false;
        EME.State.active_batch_name = batch_data.name;
        EME.State.active_batch_title = batch_data.title || '';
        EME.State.active_batch_description = batch_data.description || '';

        var items = batch_data.items || [];
        // نعكس المصفوفة لأن add_row يضيف في البداية
        items.reverse().forEach(function (item) {
            var row_obj = {
                docname: item.execution_file || '',
                ...
                is_missing: !item.execution_file, // إذا لم يكن هناك ملف حقيقي
                is_dirty: false,
                dirty_fields: {}
            };
            EME.Store.add_row(row_obj);
            EME.State.scanned_references.add(item.reference);
        });

        EME.UI.renderGrid();

        if (EME.State.is_readonly_mode) {
        ...
        this.update_batch_indicator(batch_data.title, EME.State.is_readonly_mode);
    },
    ...
    action_save_draft: function () {
        ...
        if (EME.State.active_batch_name) {
            this._do_save_draft(EME.State.active_batch_name, EME.State.active_batch_title,
                EME.State.active_batch_description, items);
            return;
        }
        this._show_batch_title_dialog(EME.State.active_batch_title, EME.State.active_batch_description,
            function (title, desc) {
                EME.Batch._do_save_draft(EME.State.active_batch_name, title, desc, items);
            });
    },
    ...
    _show_batch_title_dialog: function (def_title, def_desc, on_confirm) {
        var dialog = new frappe.ui.Dialog({
            ...
            fields: [
                { fieldtype: 'Data', fieldname: 'title', label: __('عنوان المحفظة البريدية'), reqd: 1, default: def_title || 'بريد ملفات التنفيذ ' + new Date().toLocaleDateString('ar-YE') },
    ...
    _do_save_draft: function (batch_name, title, desc, items) {
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.save_batch_draft',
            args: {
            ...
            callback: function (r) {
                if (r.message && r.message.success) {
                    EME.State.active_batch_name = r.message.batch_name;
                    EME.State.active_batch_title = r.message.title;
                    EME.State.active_batch_description = desc || '';
                    ...
                    EME.Batch.update_batch_indicator(r.message.title, false);
                    $('#action-save-draft').hide();
    ...
    action_fetch_from_archive: function () {
        $.when(
            frappe.call({ method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.get_draft_batches' }),
            frappe.call({ method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.get_posted_batches' })
            ...
            EME.Batch._show_batch_selection_dialog(__('اختر محفظة بريدية'), all, function (chosen) {
                var is_posted_batch = chosen.status === 'Posted';
                frappe.call({
                    method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.load_batch_data',
                    args: { batch_name: chosen.name },
                    freeze: true, freeze_message: __('جاري تحميل المحفظة البريدية...'),
                    callback: function (r2) {
                        if (r2.message) EME.Batch.load_batch_into_grid(r2.message, is_posted_batch);
    ...
    action_post_batch: function () {
        ...
        frappe.confirm(__('هل أنت متأكد من ترحيل المحفظة البريدية؟ سيتم إضافة {0} سجل بريد إلى ملفات التنفيذ.', [items.length]), function () {
            if (EME.State.active_batch_name) {
                EME.Batch._do_post_batch(EME.State.active_batch_name, EME.State.active_batch_title,
                    EME.State.active_batch_description, items);
            } else {
                EME.Batch._show_batch_title_dialog(EME.State.active_batch_title, EME.State.active_batch_description,
                    function (title, desc) {
                        EME.Batch._do_post_batch(null, title, desc, items);
                    });
            }
        });
    },
    ...
    _do_post_batch: function (batch_name, title, desc, items) {
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.post_batch_archive',
            args: {
                batch_name: batch_name || '',
                items: JSON.stringify(items),
                title: title || '',
            ...
            callback: function (r) {
                if (r.message && r.message.success) {
                    ...
                    EME.State.active_batch_title = r.message.title;
                    EME.State.active_batch_description = desc || '';
                    EME.State.is_readonly_mode = true;
                    ...
                    EME.Batch.update_batch_indicator(r.message.title, true);
...
// =============================================================================
// EME.Print — الطباعة (مطابق لـ EBE.Print في v2)
// =============================================================================
...
EME.Print = {
    action_print: function () {
        ...
        dialog.show();
        dialog.set_values({
            judge_name: EME.State.active_judge_name || '',
    ...
    _do_print: function (opts) {
        ...
        var rows_data = [];
        var row_num = 1;
        var source_data = opts.data || EME.Store.get_all();
        source_data.forEach(function (r) {
            ...
            rows_data.push({
                ...
                respondent: r.respondent || '',
                judge: r.judge || '',
                assistant: r.execution_assistant || '',
                cur_asst: r.current_execution_assistant || '',
                status: r.status || '',
        ...
        var report_title = __('سجلات البريد لملفات التنفيذ');
        ...
        var theadContent = '<thead>'
            ...
            + '<th>' + __('حالة الملف') + '</th><th>' + __('حالة البريد') + '</th><th>' + __('رفع البريد') + '</th>'
    ...
    action_print_all: function () {
        var dialog = new frappe.ui.Dialog({
            ...
            primary_action: function (values) {
                ...
                dialog.hide();
                frappe.call({
                    method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.get_files_with_any_mail_status',
                    args: {
...
EME.Events = {
    setup: function (page) {
        ...
        // زر ترحيل المحفظة
        $('#mail-btn-post').on('click', function () { EME.Batch.action_post_batch(); });

        // حفظ المحفظة
        $(document).on('click', '#action-save-draft', function (e) { e.preventDefault(); EME.Batch.action_save_draft(); });

        // إضافة رد (لمحفظة مرحل)
        $(document).on('click', '#action-add-reply', function (e) {
            e.preventDefault();
            EME.State.is_reply_mode = true;
            $('#li-add-reply').hide();
            $('#li-fetch-mail-completed').hide();
            ...
            EME.State.records.forEach(function (row) {
        ...
        // جلب من الأرشيف
        $(document).on('click', '#action-fetch-from-archive', function (e) { e.preventDefault(); EME.Batch.action_fetch_from_archive(); });

        // جلب الملفات المستكمِل إطلاعها
        $(document).on('click', '#action-fetch-mail-completed', function (e) {
            e.preventDefault();
            frappe.call({
                method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.get_files_with_mail_status',
                args: {
                    file_status: 'منظور',
                ...
                callback: function (r) {
                    if (r.message && r.message.length > 0) {
                        r.message.forEach(function (item) {
                            var exists = EME.State.records.some(function (r) { return r.reference === item.reference; });
                            if (!exists) {
                                EME.Logic.apply_new_row_rules(item);
                                EME.Store.add_row(item);
                            }
                        });
                        EME.UI.renderGrid();
                        frappe.show_alert({
        ...
        // طباعة
        $(document).on('click', '#action-print', function (e) { e.preventDefault(); EME.Print.action_print(); });
        $(document).on('click', '#action-print-all', function (e) { e.preventDefault(); EME.Print.action_print_all(); });

        // حذف محفظة بريد غير مرحل
        $(document).on('click', '#btn-delete-archive', function (e) {
            e.preventDefault();
            if (!EME.State.active_batch_name) return;
            frappe.confirm(
                __('هل أنت متأكد من حذف المحفظة البريدية "{0}"؟', [EME.State.active_batch_name]),
                function () {
                    frappe.call({
                        method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.delete_draft_batch',
                        args: { batch_name: EME.State.active_batch_name },
                        callback: function (r) {
                            if (r.message && r.message.success) {
        ...
        $('#action-edit-batch').on('click', function (e) {
            ...
            EME.Batch._show_batch_title_dialog(
                EME.State.active_batch_title, EME.State.active_batch_description,
                function (title, desc) {
                    var items = EME.Batch.collect_current_grid_items();
                    EME.Batch._do_save_draft(EME.State.active_batch_name, title, desc, items);
                }
            );
        });
        ...
        $(document).on('change', '.cell-input, .ref-edit-input', function () {
            ...
            if (input.hasClass('ref-edit-input')) {
                EME.Store.update_row_field(id, 'reference', input.val());
                return;
            }
            ...
            var row = EME.Store.update_row_field(id, fieldname, val);
            if (row) {
                ...
                row = EME.Logic.resolve_dependencies(row, fieldname);
                EME.Grid.mark_batch_as_dirty();
        ...
        $('#pagination-page-size').on('change', function () {
            var val = $(this).val();
            EME.State.records_per_page = (val === 'all') ? Infinity : (parseInt(val) || 25);
            EME.State.current_page = 1;
            EME.UI.reindex_rows();
        });
        ...
        $('#pagination-btn-prev').on('click', function () {
            if (EME.State.current_page > 1) { EME.State.current_page--; EME.UI.apply_pagination(); }
        });
        $('#pagination-btn-next').on('click', function () {
            var total_records = $('#mail-results-body tr').not('#empty-row').length;
            var total_pages = Math.ceil(total_records / EME.State.records_per_page);
            if (EME.State.current_page < total_pages) { EME.State.current_page++; EME.UI.apply_pagination(); }
        });
        ...
        $(document).on('click', '.sortable-header', function () {
            ...
            rows.sort(function (a, b) {
                var valA = EME.Events._get_cell_sorting_value($(a).find('td:nth-child(' + col_idx + ')'));
                var valB = EME.Events._get_cell_sorting_value($(b).find('td:nth-child(' + col_idx + ')'));
                var isNum = /^\d+(\.\d+)?$/;
        ...
        $('#mail-reference-input').on('keypress', function (e) {
            if (e.which === 13) { e.preventDefault(); EME.Grid.perform_search(); }
        });

        // Enter لتصحيح المرجع
        $(document).on('keydown', '.ref-edit-input', function (e) {
            if (EME.State.is_readonly_mode) return;
            if (e.which === 13) { e.preventDefault(); EME.Grid.research_row_reference($(this).closest('tr')); }
        });
        ...
        $(document).on('keydown', '.row-missing .cell-input[data-fieldname="file_number"], .row-missing .cell-input[data-fieldname="year"], .row-missing .cell-input[data-fieldname="petitioner"], .row-missing .cell-input[data-fieldname="respondent"]', function (e) {
            if (EME.State.is_readonly_mode) return;
            if (e.which === 13) { e.preventDefault(); EME.Grid.trigger_row_search_lookup($(this).closest('tr')); }
        });
        ...
        $(document).on('click', '.btn-mail-delete-row', function () {
            ...
            if (checked.length > 0 && is_checked) {
                frappe.confirm(__('هل أنت متأكد من حذف الصفوف المحددة ({0})؟', [checked.length]), function () {
                    checked.each(function () { EME.Grid.delete_row_logic($(this).closest('tr')); });
                    EME.UI.reindex_rows();
                    EME.UI.update_counter();
                    $('#select-all-rows').prop('checked', false);
                    EME.UI.check_empty_table();
                });
            } else {
                EME.Grid.delete_row_logic(tr);
                EME.UI.reindex_rows();
                EME.UI.update_counter();
                EME.UI.update_select_all_state();
                EME.UI.check_empty_table();
            }
        });
...
// =============================================================================
// EME.CSS + EME.UI_TEMPLATE — الأنماط والقوالب
// =============================================================================
...
EME.CSS = ''
    ...
    + '#editor-table thead th:nth-child(12){width:' + COL_WIDTHS.mail_status + 'px;min-width:' + COL_WIDTHS.mail_status + 'px}'
    + '#editor-table thead th:nth-child(13){width:' + COL_WIDTHS.upload_mail + 'px;min-width:' + COL_WIDTHS.upload_mail + 'px}'
    + '#editor-table thead th:nth-child(14){width:' + COL_WIDTHS.upload_mail_date + 'px;min-width:' + COL_WIDTHS.upload_mail_date + 'px}'
    + '#editor-table thead th:nth-child(15){width:' + COL_WIDTHS.mail_returned + 'px;min-width:' + COL_WIDTHS.mail_returned + 'px}'
    + '#editor-table thead th:nth-child(16){width:' + COL_WIDTHS.mail_returned_date + 'px;min-width:' + COL_WIDTHS.mail_returned_date + 'px}'
    + '#editor-table thead th:nth-child(17){width:' + COL_WIDTHS.mail_notes + 'px;min-width:' + COL_WIDTHS.mail_notes + 'px}'
...
EME.UI_TEMPLATE = function () {
    return '<style>' + EME.CSS + '</style><div class="mail-container">'
        ...
        + '<tr id="empty-row"><td colspan="19" class="text-center text-muted">' + __('لا توجد سجلات بريد حالياً. ابدأ بإدخال المرجع للبحث.') + '</td>'
        + '</tbody>'
        + '</table></div>'
...
frappe.pages['execution-mail-editor'].on_page_load = function (wrapper) {
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __('محرر البريد لملفات التنفيذ | Execution Mail Editor'),
        single_column: true
    });

    // تحميل حالات البريد أولاً، ثم بناء الواجهة
    frappe.call({
        method: 'judicial_files.judicial_files.page.execution_mail_editor.execution_mail_editor.get_mail_statuses',
        callback: function (r) {
            if (r.message) {
                EME.State.mail_statuses = r.message;
            }
            $(page.body).html(EME.UI_TEMPLATE());
            EME.Events.setup(page);
        }
    });
};