// =============================================================================
// execution_batch_editor_v2.js — محرر دفعات ملفات التنفيذ المطور
// =============================================================================
// منظمة في Namespaces:
//   EBE.State    → الحالة العامة
//   EBE.UI       → دوال الواجهة المساعدة (عدادات، تصفح، ترتيب)
//   EBE.Grid     → إدارة الجدول (إضافة، بحث، تعديل، حذف)
//   EBE.Batch    → إدارة الأرشفة (حفظ، تحميل، ترحيل)
//   EBE.Print    → الطباعة
//   EBE.Events   → ربط الأحداث
//   EBE.init     → نقطة البداية
// =============================================================================
...
// =============================================================================
// EBE.State — الحالة العامة لمحرر الدفعات
// =============================================================================
...
EBE.UI = {
    ...
    apply_pagination: function () {
        ...
        var is_all = (EBE.State.records_per_page === Infinity || isNaN(EBE.State.records_per_page));
        var total_pages = is_all ? 1 : Math.ceil(total_records / EBE.State.records_per_page);

        if (EBE.State.current_page > total_pages) EBE.State.current_page = total_pages || 1;
        if (EBE.State.current_page < 1) EBE.State.current_page = 1;

        var start_idx = is_all ? 1 : (EBE.State.current_page - 1) * EBE.State.records_per_page + 1;
        var end_idx = is_all ? total_records : Math.min(EBE.State.current_page * EBE.State.records_per_page, total_records);
        ...
        var page_info = is_all
            ...
            : __('إجمالي') + ': <strong>' + total_records + '</strong> ' + __('سجل') + ' | ' + __('صفحة') + ' <strong>' + EBE.State.current_page + '</strong> ' + __('من') + ' <strong>' + total_pages + '</strong>';
        $('#pagination-info').html(page_info);

        $('#pagination-btn-prev').prop('disabled', is_all || EBE.State.current_page <= 1)
            .css({ opacity: (is_all || EBE.State.current_page <= 1) ? '0.5' : '1', cursor: (is_all || EBE.State.current_page <= 1) ? 'not-allowed' : 'pointer' });
        $('#pagination-btn-next').prop('disabled', is_all || EBE.State.current_page >= total_pages)
            .css({ opacity: (is_all || EBE.State.current_page >= total_pages) ? '0.5' : '1', cursor: (is_all || EBE.State.current_page >= total_pages) ? 'not-allowed' : 'pointer' });
    ...
    reindex_rows: function () {
        ...
        EBE.State.scan_index = current_row - 1;
    ...
    check_empty_table: function () {
        ...
        if (rows.length === 0 || (rows.length === 1 && rows.attr('id') === 'empty-row')) {
            EBE.State.scan_index = 0;
            $('#scanner-results-body').html(
                '<tr id="empty-row"><td colspan="15" class="text-center text-muted" style="padding: 50px; font-size: 14px; text-align: center;">'
    ...
    update_new_status_cell_style: function (tr) {
        ...
        var current_asst_cell = tr.find('.cell-input[data-fieldname="current_execution_assistant"]');
        ...
        if (new_status_val === 'مرحل') {
            ...
            if (!current_post_val || !EBE.State.posting_types.includes(current_post_val)) {
                var default_val = EBE.State.posting_types.includes('حفظ') ? 'حفظ' : (EBE.State.posting_types.length > 0 ? EBE.State.posting_types[0] : 'حفظ');
                posting_type_cell.val(default_val).addClass('is-dirty');
            }
            posting_type_cell.css(archived_style);
            if (current_asst_cell.length) current_asst_cell.css(archived_style);
            ...
            var current_post_val = posting_type_cell.val() ? posting_type_cell.val().trim() : '';
            if (EBE.State.posting_types.includes(current_post_val) && !EBE.State.posting_types.includes(original_posting)) {
            ...
            if (current_asst_cell.length) {
                if (current_asst_cell.hasClass('is-dirty')) current_asst_cell.css(normal_style_dirty);
                else current_asst_cell.css(normal_style_clean);
            }
...
EBE.Grid = {
    ...
    mark_batch_as_dirty: function () {
        if (EBE.State.is_readonly_mode) return;
    ...
    perform_search: function () {
        ...
        if (EBE.State.scanned_references.has(reference)) {
        ...
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.get_execution_file',
            args: { reference: reference },
            callback: function (r) {
                if (r.message) {
                    EBE.Grid.add_row_to_grid(r.message, true);
                    EBE.State.scanned_references.add(reference);
                    frappe.show_alert({ message: __('تم العثور على الملف وجلب بياناته بنجاح.'), indicator: 'green' });
                } else {
                    EBE.Grid.add_row_to_grid({
    ...
    add_row_to_grid: function (file, exists) {
        $('#empty-row').remove();
        EBE.State.scan_index++;

        file.original_current_execution_assistant = file.original_current_execution_assistant !== undefined
            ? file.original_current_execution_assistant : (file.current_execution_assistant || '');

        var selected_assistant = EBE.State.assistant_control ? EBE.State.assistant_control.get_value() : '';
        if (selected_assistant && file.status === 'منظور') {
            file.current_execution_assistant = selected_assistant;
        }
        ...
        EBE.State.posting_types.forEach(function (pt) {
            posting_type_options += '<option value="' + pt + '" ' + (file.posting_type === pt ? 'selected' : '') + '>' + pt + '</option>';
        });

        // التعيين التلقائي لمعاون التنفيذ الحالي
        var current_asst_val = file.current_execution_assistant || '';
        var current_asst_dirty = (current_asst_val !== file.original_current_execution_assistant) ? 'is-dirty' : '';
        var current_asst_style = (current_asst_val !== file.original_current_execution_assistant) ? 'background-color: #fffbeb; font-weight: bold;' : '';

        var row_html = '<tr id="scan-row-' + EBE.State.scan_index + '" class="' + row_class + '">'
            + '<td class="sticky-col-right-1 td-center">'
            + '<input type="checkbox" class="row-selector-checkbox" title="' + __('تحديد هذا الصف') + '"></td>'
            + '<td class="sticky-col-right-2 td-center td-index">' + EBE.State.scan_index + '</td>'
            + '<td class="sticky-col-right-3 td-ref">' + file_link_html + '</td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.file_number || '') + '" data-original="' + (file.file_number || '') + '" data-row="' + EBE.State.scan_index + '" data-col="1" data-docname="' + (file.name || '') + '" data-fieldname="file_number" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.year || '') + '" data-original="' + (file.year || '') + '" data-row="' + EBE.State.scan_index + '" data-col="2" data-docname="' + (file.name || '') + '" data-fieldname="year" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.petitioner || '') + '" data-original="' + (file.petitioner || '') + '" data-row="' + EBE.State.scan_index + '" data-col="3" data-docname="' + (file.name || '') + '" data-fieldname="petitioner" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.respondent || '') + '" data-original="' + (file.respondent || '') + '" data-row="' + EBE.State.scan_index + '" data-col="4" data-docname="' + (file.name || '') + '" data-fieldname="respondent" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.judge || '') + '" data-original="' + (file.judge || '') + '" data-row="' + EBE.State.scan_index + '" data-col="5" data-docname="' + (file.name || '') + '" data-fieldname="judge" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.execution_assistant || '') + '" data-original="' + (file.execution_assistant || '') + '" data-row="' + EBE.State.scan_index + '" data-col="6" data-docname="' + (file.name || '') + '" data-fieldname="execution_assistant" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + ' ' + current_asst_dirty + '" value="' + current_asst_val + '" data-original="' + (file.original_current_execution_assistant || '') + '" data-row="' + EBE.State.scan_index + '" data-col="7" data-docname="' + (file.name || '') + '" data-fieldname="current_execution_assistant" style="' + current_asst_style + '" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.status || '') + '" data-original="' + (file.status || '') + '" data-row="' + EBE.State.scan_index + '" data-col="8" data-docname="' + (file.name || '') + '" data-fieldname="status" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + new_status_val + '" data-original="' + new_status_val + '" data-row="' + EBE.State.scan_index + '" data-col="9" data-docname="' + (file.name || '') + '" data-fieldname="new_status" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><select class="' + input_style + '" data-original="' + (file.posting_type || '') + '" data-row="' + EBE.State.scan_index + '" data-col="10" data-docname="' + (file.name || '') + '" data-fieldname="posting_type" ' + readonly_attr + '><option value=""></option>' + posting_type_options + '</select></td>'
            + '<td class="td-cell date-picker-wrapper"><input type="text" data-fieldtype="Date" class="' + input_style + ' date-input" value="' + frappe.datetime.str_to_user(file.archive_date || '') + '" data-sys-date="' + (file.archive_date || '') + '" data-original="' + (file.archive_date || '') + '" data-row="' + EBE.State.scan_index + '" data-col="11" data-docname="' + (file.name || '') + '" data-fieldname="archive_date" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.archive_year_hijri || '') + '" data-original="' + (file.archive_year_hijri || '') + '" data-row="' + EBE.State.scan_index + '" data-col="12" data-docname="' + (file.name || '') + '" data-fieldname="archive_year_hijri" ' + readonly_attr + '></td>'
            + '<td class="td-cell"><input type="text" class="' + input_style + '" value="' + (file.archive_month_hijri || '') + '" data-original="' + (file.archive_month_hijri || '') + '" data-row="' + EBE.State.scan_index + '" data-col="13" data-docname="' + (file.name || '') + '" data-fieldname="archive_month_hijri" ' + readonly_attr + '></td>'
            ...
            + '<button class="btn-toggle-posting" ' + (EBE.State.is_readonly_mode ? 'disabled' : '') + ' title="' + __('تحويل نوع الترحيل') + '" style="padding:1px 4px;border-radius:3px;border:1px solid #ccc;color:#95a5a6;background:transparent;cursor:' + (EBE.State.is_readonly_mode ? 'not-allowed' : 'pointer') + ';opacity:' + (EBE.State.is_readonly_mode ? '0.4' : '1') + ';margin-left:2px;font-size:10px;vertical-align:middle;"><i class="fa fa-exchange" style="font-size:10px;"></i></button>'
        ...
        EBE.UI.update_new_status_cell_style(new_tr);

        EBE.State.current_page = 1;
        EBE.UI.reindex_rows();
        EBE.UI.update_counter();
        EBE.UI.add_focus_styling();
        EBE.UI.init_date_pickers();
    ...
    research_row_reference: function (tr) {
        ...
        frappe.call({
            ...
            callback: function (r) {
                ...
                if (r.message) {
                    var file = r.message;
                    if (old_ref) EBE.State.scanned_references.delete(old_ref);
                    EBE.State.scanned_references.add(new_ref);
                    ...
                    EBE.UI.update_new_status_cell_style(tr);
    ...
    delete_row_logic: function (row) {
        ...
        if (ref) EBE.State.scanned_references.delete(ref);
        row.remove();
        this.mark_batch_as_dirty();
    },
    ...
    trigger_row_search_lookup: function (tr) {
        ...
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.search_execution_files',
            args: { file_number: file_number, year: year, petitioner: petitioner, respondent: respondent },
            callback: function (r) {
                if (r.message && r.message.length > 0) {
                    if (r.message.length === 1) {
                        EBE.Grid.populate_tr_with_file(tr, r.message[0]);
                    } else {
                        EBE.Grid.show_selection_dialog(tr, r.message);
                    }
                } else {
    ...
    populate_tr_with_file: function (tr, file) {
        ...
        if (old_ref) EBE.State.scanned_references.delete(old_ref);
        EBE.State.scanned_references.add(file.reference);
    ...
    show_selection_dialog: function (tr, files) {
        ...
        var dialog = new frappe.ui.Dialog({
            ...
            fields: [{
                ...
                options: '<div style="max-height:350px;overflow-y:auto;direction:rtl;text-align:right;">'
                    ...
                    + '<thead><tr style="background:#f8f9fa;"><th>' + __('المرجع') + '</th><th>' + __('رقم الملف') + '</th><th>' + __('السنة') + '</th><th>' + __('طالب التنفيذ') + '</th><th>' + __('المنفذ ضده') + '</th></tr></thead>'
        ...
        $(document).off('click', '#dialog-lookup-table tbody tr').on('click', '#dialog-lookup-table tbody tr', function () {
            ...
            if (files[idx]) EBE.Grid.populate_tr_with_file(tr, files[idx]);
            dialog.hide();
        });
    },
    ...
    save_all_changes: function () {
        ...
        if (!selected_assistant) {
            frappe.show_alert({ message: __('يرجى تحديد معاون التنفيذ المختص من الحقل العلوي أولاً.'), indicator: 'red' });
            if (EBE.State.assistant_control && EBE.State.assistant_control.wrapper) {
                EBE.State.assistant_control.wrapper.find('input').focus();
            }
            return;
        }
        ...
        $('#scanner-results-body tr').each(function () {
            ...
            changes.push({
                ...
                current_execution_assistant: selected_assistant,
        ...
        frappe.confirm(__('هل أنت متأكد من إتمام عملية حفظ لعدد ({0}) سجلات؟', [changes.length]), function () {
            frappe.call({
                method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.save_grid_changes',
    ...
    // --- التعيين التلقائي لمعاون التنفيذ ---
    apply_auto_assistant_assignment: function (assistant_name) {
        if (!assistant_name) return;
        $('#scanner-results-body tr').each(function () {
            var tr = $(this);
            if (tr.attr('id') === 'empty-row') return;
            var status_val = tr.find('.cell-input[data-fieldname="status"]').val() || '';
            if (status_val === 'منظور') {
                var cell = tr.find('.cell-input[data-fieldname="current_execution_assistant"]');
                if (cell.length && cell.val() !== assistant_name) {
                    cell.val(assistant_name).addClass('is-dirty').css({ 'background-color': '#fffbeb', 'font-weight': 'bold' });
                }
            }
        });
    },
    ...
    apply_auto_archive_date: function (archive_date) {
        ...
        judicial_files.utils.get_hijri_date(archive_date).then(function (hijri) {
            if (!hijri) return;
            EBE.State.active_archive_date = archive_date;
            EBE.State.active_archive_date_hijri = hijri.date || '';
            EBE.State.active_archive_year_hijri = hijri.year ? hijri.year.toString() : '';
            EBE.State.active_archive_month_hijri = hijri.month_name || '';
...
EBE.Batch = {
    ...
    collect_current_grid_items: function () {
        ...
        $('#scanner-results-body tr').each(function () {
            ...
            items.push({
                reference: reference,
                execution_file: tr.find('.cell-input[data-fieldname="file_number"]').data('docname') || '',
                ...
                original_assistant: tr.find('.cell-input[data-fieldname="execution_assistant"]').val() || '',
                current_assistant: tr.find('.cell-input[data-fieldname="current_execution_assistant"]').val() || '',
    ...
    // --- مسح الجدول ---
    clear_grid: function () {
        $('#scanner-results-body').html('<tr id="empty-row"><td colspan="15" class="text-center text-muted" style="padding: 60px; font-size: 14px; text-align: center;">'
            + __('لا توجد ملفات حالياً. ابدأ بإدخال المرجع للبدء في التعديل السريع.') + '</td></tr>');
        EBE.State.scan_index = 0;
        EBE.State.scanned_references = new Set();
        EBE.State.active_batch_name = null;
        EBE.State.active_batch_title = '';
        EBE.State.active_batch_description = '';
        EBE.State.active_logo_emblem = '';
        EBE.State.active_header_text_image = '';
        EBE.State.active_judge_name = '';
        EBE.State.active_archive_month_hijri = '';
        EBE.State.is_readonly_mode = false;
        EBE.UI.update_counter();
        $('#table-pagination-bar').hide();
    ...
    update_batch_indicator: function (title, is_posted, mode_label) {
        $('#active-batch-indicator').remove();
        console.log('[EBE] update_batch_indicator — title:', title, 'is_posted:', is_posted, 'mode_label:', mode_label);
        if (!title) return;
        var color = '#27ae60';
        var icon = 'fa-archive';
    ...
    load_batch_into_grid: function (batch_data, readonly) {
        this.clear_grid();
        EBE.State.is_readonly_mode = !!readonly;
        EBE.State.active_batch_name = batch_data.name;
        EBE.State.active_batch_title = batch_data.title || '';
        EBE.State.active_batch_description = batch_data.description || '';
        EBE.State.active_logo_emblem = batch_data.logo_emblem || '';
        EBE.State.active_header_text_image = batch_data.header_text_image || '';
        EBE.State.active_judge_name = batch_data.judge_name || '';
        EBE.State.active_archive_month_hijri = batch_data.archive_month_hijri || '';
        EBE.State.active_archive_date = '';
        EBE.State.active_archive_date_hijri = '';
        EBE.State.active_archive_year_hijri = '';

        (batch_data.items || []).forEach(function (item, idx) {
            if (idx === 0 && item.archive_date) {
                EBE.State.active_archive_date = item.archive_date;
                EBE.State.active_archive_date_hijri = item.archive_date_hijri || '';
                EBE.State.active_archive_year_hijri = item.archive_year_hijri || '';
                EBE.State.active_archive_month_hijri = item.archive_month_hijri || EBE.State.active_archive_month_hijri;
            }
            var file_obj = {
                name: item.execution_file || '',
                ...
                judge: item.judge || '',
                execution_assistant: item.original_assistant || '',
                current_execution_assistant: item.current_assistant || '',
                original_current_execution_assistant: item.current_assistant || '',
            ...
            EBE.Grid.add_row_to_grid(file_obj, !!item.execution_file);
            EBE.State.scanned_references.add(item.reference);
        });

        if (EBE.State.is_readonly_mode) {
            ...
            $('#scanner-assistant-container input').prop('disabled', true).css('cursor', 'not-allowed');
        ...
        this.update_batch_indicator(batch_data.title, EBE.State.is_readonly_mode);
    },
    ...
    action_save_draft: function () {
        ...
        if (EBE.State.active_batch_name) {
            this._do_save_draft(EBE.State.active_batch_name, EBE.State.active_batch_title,
                EBE.State.active_batch_description, EBE.State.active_logo_emblem,
                EBE.State.active_header_text_image, EBE.State.active_judge_name,
                EBE.State.active_archive_date, EBE.State.active_archive_date_hijri,
                EBE.State.active_archive_year_hijri, EBE.State.active_archive_month_hijri, items);
            return;
        }
        this._show_batch_title_dialog(EBE.State.active_batch_title, EBE.State.active_batch_description,
            EBE.State.active_logo_emblem, EBE.State.active_header_text_image,
            EBE.State.active_judge_name, EBE.State.active_archive_date,
            EBE.State.active_archive_date_hijri, EBE.State.active_archive_year_hijri,
            EBE.State.active_archive_month_hijri,
            function (title, desc, logo, header, judge, ad, adh, ayh, month) {
                EBE.State.active_archive_date = ad || EBE.State.active_archive_date;
                EBE.State.active_archive_date_hijri = adh || EBE.State.active_archive_date_hijri;
                EBE.State.active_archive_year_hijri = ayh || EBE.State.active_archive_year_hijri;
                EBE.State.active_archive_month_hijri = month || EBE.State.active_archive_month_hijri;
                EBE.Batch._do_save_draft(EBE.State.active_batch_name, title, desc, logo, header, judge, ad, adh, ayh, month, items);
            });
    },
    ...
    _show_batch_title_dialog: function (def_title, def_desc, def_logo, def_header, def_judge, def_ad, def_adh, def_ayh, def_month, on_confirm) {
        function _set_title_from(dialog, month, year) {
            var base = 'ارشيف ملفات التنفيذ شهر';
            var asst = EBE.State.assistant_control ? EBE.State.assistant_control.get_value() : '';
            if (month && year) {
                dialog.set_value('title', base + ' ' + month + ' لعام ' + year + ' هـ' + (asst ? ' - معاون: ' + asst : ''));
        ...
        var dialog = new frappe.ui.Dialog({
            ...
            fields: [
                { fieldtype: 'Data', fieldname: 'title', label: __('عنوان المحفظة'), reqd: 1, read_only: 1, default: def_title || 'ارشيف ملفات التنفيذ شهر' },
                { fieldtype: 'Date', fieldname: 'archive_date', label: __('تاريخ الأرشفة ميلادي'), reqd: 1, default: def_ad || EBE.State.active_archive_date || '', onchange: function () {
                    var value = this.get_value();
                    if (!value) { dialog.set_value('archive_date_hijri', ''); dialog.set_value('archive_year_hijri', ''); dialog.set_value('archive_month_hijri', ''); dialog.set_value('title', 'ارشيف ملفات التنفيذ شهر'); return; }
                    judicial_files.utils.get_hijri_date(value).then(function (hijri) {
                        if (hijri) {
                            ...
                            EBE.Grid.apply_auto_archive_date(value);
                ...
                { fieldtype: 'Data', fieldname: 'archive_date_hijri', label: __('تاريخ الأرشفة هجري'), read_only: 1, default: def_adh || EBE.State.active_archive_date_hijri || '' },
                { fieldtype: 'Data', fieldname: 'archive_year_hijri', label: __('سنة الأرشفة هجري'), read_only: 1, default: def_ayh || EBE.State.active_archive_year_hijri || '' },
                { fieldtype: 'Data', fieldname: 'archive_month_hijri', label: __('الشهر الهجري'), read_only: 1, default: def_month || EBE.State.active_archive_month_hijri || '' },
            ...
            primary_action: function (values) {
                var assistant_val = EBE.State.assistant_control ? EBE.State.assistant_control.get_value() : '';
                if (!assistant_val) {
                    frappe.show_alert({ message: __('يجب اختيار معاون التنفيذ المختص أولاً.'), indicator: 'red' });
                    return;
                }
        ...
        if (!def_judge) {
            EBE.UI.get_chief_justice_name(function (chief) {
    ...
    _do_save_draft: function (batch_name, title, desc, logo, header, judge, ad, adh, ayh, month, items) {
        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.save_batch_draft',
            args: {
            ...
            callback: function (r) {
                if (r.message && r.message.success) {
                    EBE.State.active_batch_name = r.message.batch_name;
                    EBE.State.active_batch_title = r.message.title;
                    EBE.State.active_batch_description = r.message.description || '';
                    EBE.State.active_logo_emblem = logo || '';
                    EBE.State.active_header_text_image = header || '';
                    EBE.State.active_judge_name = judge || '';
                    EBE.State.active_archive_date = ad || EBE.State.active_archive_date;
                    EBE.State.active_archive_date_hijri = adh || EBE.State.active_archive_date_hijri;
                    EBE.State.active_archive_year_hijri = ayh || EBE.State.active_archive_year_hijri;
                    EBE.State.active_archive_month_hijri = month || '';
                    EBE.Batch.update_batch_indicator(r.message.title, false);
                    $('#action-save-draft').hide();
    ...
    action_fetch_from_archive: function () {
        $.when(
            frappe.call({ method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.get_draft_batches' }),
            frappe.call({ method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.get_posted_batches' })
            ...
            EBE.Batch._show_batch_selection_dialog(__('اختر أرشيفاً'), all, function (chosen) {
                var is_posted_batch = chosen.status === 'Posted';
                frappe.call({
                    method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.load_batch_data',
                    args: { batch_name: chosen.name },
                    freeze: true, freeze_message: __('جاري تحميل الأرشيف...'),
                    callback: function (r2) {
                        if (r2.message) EBE.Batch.load_batch_into_grid(r2.message, is_posted_batch);
    ...
    action_post_batch: function () {
        ...
        var selected_assistant = EBE.State.assistant_control ? EBE.State.assistant_control.get_value() : '';
        if (!selected_assistant) {
            frappe.show_alert({ message: __('يرجى تحديد معاون التنفيذ المختص أولاً.'), indicator: 'red' });
            if (EBE.State.assistant_control && EBE.State.assistant_control.wrapper) EBE.State.assistant_control.wrapper.find('input').focus();
            return;
        }
        ...
        frappe.confirm(__('هل أنت متأكد من ترحيل ملف الأرشيف؟ سيتم تحديث {0} سجلاً بشكل نهائي.', [eligible.length]), function () {
            if (EBE.State.active_batch_name) {
                EBE.Batch._do_post_batch(EBE.State.active_batch_name, EBE.State.active_batch_title,
                    EBE.State.active_batch_description, EBE.State.active_logo_emblem,
                    EBE.State.active_header_text_image, EBE.State.active_judge_name,
                    EBE.State.active_archive_date, EBE.State.active_archive_date_hijri,
                    EBE.State.active_archive_year_hijri, EBE.State.active_archive_month_hijri, items);
            } else {
                EBE.Batch._show_batch_title_dialog(EBE.State.active_batch_title, EBE.State.active_batch_description,
                    EBE.State.active_logo_emblem, EBE.State.active_header_text_image,
                    EBE.State.active_judge_name, EBE.State.active_archive_date,
                    EBE.State.active_archive_date_hijri, EBE.State.active_archive_year_hijri,
                    EBE.State.active_archive_month_hijri,
                    function (title, desc, logo, header, judge, ad, adh, ayh, month) {
                        ...
                        EBE.State.active_archive_year_hijri = ayh || EBE.State.active_archive_year_hijri;
                        EBE.State.active_archive_month_hijri = month || EBE.State.active_archive_month_hijri;
                        EBE.Batch._do_post_batch(null, title, desc, logo, header, judge, ad, adh, ayh, month, items);
                    });
            }
        });
    },
    ...
    _do_post_batch: function (batch_name, title, desc, logo, header, judge, ad, adh, ayh, month, items) {
        var selected_assistant = EBE.State.assistant_control ? EBE.State.assistant_control.get_value() : '';
        items.forEach(function (it) { it.current_assistant = selected_assistant || it.current_assistant; });

        frappe.call({
            method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.post_batch_archive',
            args: {
                batch_name: batch_name || '', items: JSON.stringify(items),
                title: title || '', description: desc || '',
                logo_emblem: logo || '', header_text_image: header || '',
            ...
            callback: function (r) {
                if (r.message && r.message.success) {
                    ...
                    EBE.State.active_judge_name = judge || '';
                    EBE.State.active_archive_month_hijri = month || '';
                    EBE.State.is_readonly_mode = true;
                    $('#scanner-results-body .cell-input').prop('readonly', true).css('cursor', 'default');
                    $('#scanner-reference-input').prop('disabled', true);
                    $('#scanner-btn-post').prop('disabled', true).css('opacity', '0.5');
                    $('#btn-delete-archive').hide();
                    EBE.Batch.update_batch_indicator(r.message.title, true);
...
EBE.Print = {
    action_print: function () {
        ...
        var posting_type_options = [__('الكل')].concat(EBE.State.posting_types).join('\n');
        ...
        var dialog = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                { fieldtype: 'Section Break', label: __('صورة الترويسة') },
                { fieldtype: 'Attach Image', fieldname: 'print_archive_image', label: __('صورة الترويسة (عرض كامل)'), default: EBE.State.active_logo_emblem || '/files/a_archive.png' }
            ],
            ...
            primary_action: function (values) {
                ...
                EBE.Print._do_print({
                    ...
                    archive_year_hijri: EBE.State.active_archive_year_hijri || '',
                    posting_type_filter: values.posting_type_filter || __('الكل'),
                    print_logo: values.print_archive_image || EBE.State.active_logo_emblem,
                    print_header: values.print_archive_image || EBE.State.active_header_text_image
                });
            }
        });
        dialog.show();
        dialog.set_values({
            judge_name: EBE.State.active_judge_name || '',
            month_hijri: EBE.State.active_archive_month_hijri || default_month,
            print_archive_image: EBE.State.active_logo_emblem || '/files/a_archive.png'
        });
    },
    ...
    _do_print: function (opts) {
        ...
        $('#scanner-results-body tr').not('#empty-row').each(function () {
            ...
            rows_data.push({
                ...
                respondent: tr.find('.cell-input[data-fieldname="respondent"]').val() || '',
                judge: tr.find('.cell-input[data-fieldname="judge"]').val() || '',
                assistant: tr.find('.cell-input[data-fieldname="execution_assistant"]').val() || '',
                cur_asst: tr.find('.cell-input[data-fieldname="current_execution_assistant"]').val() || '',
        ...
        var report_title = __('القضايا التنفيذية المرحلة للأرشيف') + ' (' + title_posting + ')';
        ...
        var theadContent = '<thead>'
            ...
            + '<th>' + __('طالب التنفيذ') + '</th><th>' + __('المنفذ ضده') + '</th><th class="print-hide-col">' + __('القاضي') + '</th>'
            + '<th>' + __('معاون التنفيذ المختص') + '</th><th>' + __('معاون التنفيذ الحالي') + '</th>'
    ...
    action_print_all: function () {
        ...
        var dialog = new frappe.ui.Dialog({
            ...
            primary_action: function (values) {
                ...
                var args = {
                    ...
                    year: EBE.State.active_archive_year_hijri || '',
                    posting_type: (values.posting_type_filter === __('الكل')) ? '' : values.posting_type_filter
                };

                frappe.call({
                    method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.get_execution_files_for_print',
                    args: args,
                    freeze: true,
                    ...
                    callback: function (r) {
                        ...
                        data.forEach(function (row) {
                            ...
                            tbody_html += '<tr' + row_color + '>'
                                ...
                                + '<td>' + frappe.utils.escape_html(row.execution_assistant || '') + '</td>'
                                + '<td>' + frappe.utils.escape_html(row.current_execution_assistant || '') + '</td>'
                        ...
                        if (EBE.State.active_archive_year_hijri) report_title += ' ' + EBE.State.active_archive_year_hijri + 'هـ';
                        ...
                        var print_date = new Date().toLocaleDateString('ar-YE');
                        var img_src = values.print_archive_image || EBE.State.active_logo_emblem || '/files/a_archive.png';
                        var src = img_src.startsWith('/') ? window.location.origin + img_src : img_src;
        ...
        dialog.set_values({
            judge_name: EBE.State.active_judge_name || '',
            month_hijri: EBE.State.active_archive_month_hijri || '',
            print_archive_image: EBE.State.active_logo_emblem || '/files/a_archive.png'
        });
    },
};
...
// =============================================================================
// EBE.Events — ربط الأحداث (Events Setup)
// =============================================================================
...
EBE.Events = {
    setup: function (page) {
        ...
        EBE.State.assistant_control = frappe.ui.form.make_control({
            parent: $('#scanner-assistant-container'),
            df: {
                fieldtype: 'Link',
                fieldname: 'execution_assistant',
                options: 'Judicial Employee',
                placeholder: __('ابحث عن معاون التنفيذ...'),
                get_query: function () { return { filters: { designation: 'معاون تنفيذ' } }; },
                onchange: function () {
                    var val = EBE.State.assistant_control.get_value();
                    if (val) {
                        EBE.Grid.apply_auto_assistant_assignment(val);
                        EBE.Grid.mark_batch_as_dirty();
                    }
                }
            },
        ...
        // زر ترحيل المحفظة
        $('#scanner-btn-post').on('click', function () { EBE.Batch.action_post_batch(); });

        // حفظ الأرشيف
        $(document).off('click', '#action-save-draft').on('click', '#action-save-draft', function (e) { e.preventDefault(); EBE.Batch.action_save_draft(); });

        // جلب من الأرشيف
        $(document).off('click', '#action-fetch-from-archive').on('click', '#action-fetch-from-archive', function (e) { e.preventDefault(); EBE.Batch.action_fetch_from_archive(); });

        // حذف ملف أرشيف غير مرحل
        $(document).on('click', '#btn-delete-archive', function (e) {
            e.preventDefault();
            if (!EBE.State.active_batch_name) return;
            frappe.confirm(
                __('هل أنت متأكد من حذف الأرشيف "{0}"؟', [EBE.State.active_batch_name]),
                function () {
                    frappe.call({
                        method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.delete_draft_batch',
                        args: { batch_name: EBE.State.active_batch_name },
                        callback: function (r) {
                            if (r.message && r.message.success) {
        ...
        // طباعة
        $('#action-print').on('click', function (e) { e.preventDefault(); EBE.Print.action_print(); });
        $('#action-print-all').on('click', function (e) { e.preventDefault(); EBE.Print.action_print_all(); });
        ...
        $('#action-edit-batch').on('click', function (e) {
            ...
            EBE.Batch._show_batch_title_dialog(
                EBE.State.active_batch_title, EBE.State.active_batch_description,
                EBE.State.active_logo_emblem, EBE.State.active_header_text_image,
                EBE.State.active_judge_name, EBE.State.active_archive_date,
                EBE.State.active_archive_date_hijri, EBE.State.active_archive_year_hijri,
                EBE.State.active_archive_month_hijri,
                function (title, desc, logo, header, judge, ad, adh, ayh, month) {
                    var items = EBE.Batch.collect_current_grid_items();
                    EBE.Batch._do_save_draft(EBE.State.active_batch_name, title, desc, logo, header, judge, ad, adh, ayh, month, items);
                }
            );
        });
        ...
        $(document).on('change', '.cell-input[data-fieldname="archive_date"]', function () {
            ...
            judicial_files.utils.get_hijri_date(archive_date).then(function (hijri) {
                if (hijri) {
                    ...
                    if (!EBE.State.active_archive_month_hijri) EBE.State.active_archive_month_hijri = hijri.month_name;
                    EBE.Grid.mark_batch_as_dirty();
                }
            }).catch(function () {
                frappe.show_alert({ message: __('فشل تحويل تاريخ الأرشيف إلى هجري.'), indicator: 'red' });
        ...
        $('#pagination-page-size').on('change', function () {
            var val = $(this).val();
            EBE.State.records_per_page = (val === 'all') ? Infinity : (parseInt(val) || 25);
            EBE.State.current_page = 1;
            EBE.UI.reindex_rows();
        });
        ...
        $('#pagination-btn-prev').on('click', function () {
            if (EBE.State.current_page > 1) { EBE.State.current_page--; EBE.UI.apply_pagination(); }
        });
        $('#pagination-btn-next').on('click', function () {
            var total_records = $('#scanner-results-body tr').not('#empty-row').length;
            var total_pages = Math.ceil(total_records / EBE.State.records_per_page);
            if (EBE.State.current_page < total_pages) { EBE.State.current_page++; EBE.UI.apply_pagination(); }
        });
        ...
        $(document).on('click', '.sortable-header', function () {
            ...
            rows.sort(function (a, b) {
                var valA = EBE.Events._get_cell_sorting_value($(a).find('td:nth-child(' + col_idx + ')'));
                var valB = EBE.Events._get_cell_sorting_value($(b).find('td:nth-child(' + col_idx + ')'));
                var isNum = /^\d+(\.\d+)?$/;
        ...
        $('#scanner-reference-input').on('keypress', function (e) {
            if (e.which === 13) { e.preventDefault(); EBE.Grid.perform_search(); }
        });

        // Enter لتصحيح المرجع
        $(document).on('keydown', '.ref-edit-input', function (e) {
            if (e.which === 13) { e.preventDefault(); EBE.Grid.research_row_reference($(this).closest('tr')); }
        });

        // Enter للبحث في الصفوف المفقودة
        $(document).on('keydown', '.row-missing .cell-input', function (e) {
            if (e.which === 13) { e.preventDefault(); EBE.Grid.trigger_row_search_lookup($(this).closest('tr')); }
        });
        ...
        $(document).on('click', '.btn-delete-row', function () {
            ...
            if (checked.length > 0 && is_checked) {
                frappe.confirm(__('هل أنت متأكد من حذف الصفوف المحددة ({0})؟', [checked.length]), function () {
                    checked.each(function () { EBE.Grid.delete_row_logic($(this).closest('tr')); });
                    EBE.UI.reindex_rows();
                    EBE.UI.update_counter();
                    $('#select-all-rows').prop('checked', false);
                    EBE.UI.check_empty_table();
                });
            } else {
                EBE.Grid.delete_row_logic(tr);
                EBE.UI.reindex_rows();
                EBE.UI.update_counter();
                EBE.UI.update_select_all_state();
                EBE.UI.check_empty_table();
            }
        });
        ...
        $(document).on('input', '.cell-input', function () {
            ...
            if (input.data('fieldname') === 'new_status') {
                EBE.UI.update_new_status_cell_style(input.closest('tr'));
            }
        });
        ...
        $(document).off('click', '.btn-toggle-posting').on('click', '.btn-toggle-posting', function () {
            ...
            var next_val = '';
            if (EBE.State.posting_types.length > 0) {
                var current_val = posting_type_cell.val() ? posting_type_cell.val().trim() : '';
                var idx = EBE.State.posting_types.indexOf(current_val);
                next_val = (idx === -1 || idx === EBE.State.posting_types.length - 1) ? EBE.State.posting_types[0] : EBE.State.posting_types[idx + 1];
            } else {
...
// =============================================================================
// EBE.CSS + EBE.UI_TEMPLATE — الأنماط والقوالب
// =============================================================================
...
+ '#editor-table{margin-bottom:0;font-size:10.5px;width:max-content;min-width:1800px;direction:rtl;text-align:right;border-collapse:collapse}'
    ...
    + '#editor-table thead th:nth-child(9){width:' + COL_WIDTHS.exec_assistant + 'px;min-width:' + COL_WIDTHS.exec_assistant + 'px}'
    + '#editor-table thead th:nth-child(10){width:' + COL_WIDTHS.current_assistant + 'px;min-width:' + COL_WIDTHS.current_assistant + 'px}'
    ...
    + '#editor-table thead th:nth-child(14){width:' + COL_WIDTHS.archive_date + 'px;min-width:' + COL_WIDTHS.archive_date + 'px}'
    + '#editor-table thead th:nth-child(15){width:' + COL_WIDTHS.archive_year + 'px;min-width:' + COL_WIDTHS.archive_year + 'px}'
    + '#editor-table thead th:nth-child(16){width:' + COL_WIDTHS.archive_month + 'px;min-width:' + COL_WIDTHS.archive_month + 'px}'
    ...
    + '#editor-table tbody td[data-col="6"]{width:' + COL_WIDTHS.exec_assistant + 'px;min-width:' + COL_WIDTHS.exec_assistant + 'px}'
    + '#editor-table tbody td[data-col="7"]{width:' + COL_WIDTHS.current_assistant + 'px;min-width:' + COL_WIDTHS.current_assistant + 'px}'
    ...
    + '#editor-table tbody td[data-col="11"]{width:' + COL_WIDTHS.archive_date + 'px;min-width:' + COL_WIDTHS.archive_date + 'px}'
    + '#editor-table tbody td[data-col="12"]{width:' + COL_WIDTHS.archive_year + 'px;min-width:' + COL_WIDTHS.archive_year + 'px}'
    + '#editor-table tbody td[data-col="13"]{width:' + COL_WIDTHS.archive_month + 'px;min-width:' + COL_WIDTHS.archive_month + 'px}'
    ...
    // ===== حقل معاون التنفيذ =====
    + '#scanner-assistant-container .frappe-control{margin-bottom:0!important;padding:0!important}'
    + '#scanner-assistant-container .form-group{margin-bottom:0!important;padding-bottom:0!important}'
    + '#scanner-assistant-container label.control-label{display:none!important}'
    + '#scanner-assistant-container input.form-control{height:32px!important;border-radius:6px!important;border:1px solid rgba(0,0,0,0.15)!important;font-size:13px!important;background-color:#fff!important;direction:rtl;text-align:right;padding:4px 8px}'
    + '#scanner-assistant-container .link-btn{height:32px!important;top:0!important;display:flex;align-items:center;justify-content:center}'
...
EBE.UI_TEMPLATE = function () {
    return '<style>' + EBE.CSS + '</style><div class="scanner-container">'
        ...
        + '<div class="field-group"><label>' + __('معاون التنفيذ المختص') + '</label><div id="scanner-assistant-container"></div></div>'
        + '<div class="field-group"><label>' + __('رقم مرجع ملف التنفيذ (Reference)') + '</label>'
        + '<div class="input-row">'
        ...
        + '<th class="sortable-header">' + __('طالب التنفيذ') + ' <i class="fa fa-sort"></i></th>'
        + '<th class="sortable-header">' + __('المنفذ ضده') + ' <i class="fa fa-sort"></i></th>'
        + '<th class="sortable-header">' + __('القاضي المختص') + ' <i class="fa fa-sort"></i></th>'
        + '<th class="sortable-header">' + __('معاون التنفيذ المختص') + ' <i class="fa fa-sort"></i></th>'
        + '<th class="sortable-header">' + __('معاون التنفيذ الحالي') + ' <i class="fa fa-sort"></i></th>'
        ...
        + '<tr id="empty-row"><td colspan="17" class="text-center text-muted">' + __('لا توجد ملفات حالياً. ابدأ بإدخال المرجع للبدء في التعديل السريع.') + '</td></tr>'
        + '</tbody>'
        + '</table></div>'
...
frappe.pages['execution-batch-editor-v2'].on_page_load = function (wrapper) {
    var page = frappe.ui.make_app_page({
        parent: wrapper,
        title: __('محرر دفعات ملفات التنفيذ المطور | Execution Batch Editor V2'),
        single_column: true
    });

    // تحميل أنواع الترحيل أولاً، ثم بناء الواجهة
    frappe.call({
        method: 'judicial_files.judicial_files.page.execution_batch_editor_v2.execution_batch_editor_v2.get_posting_types',
        callback: function (r) {
            if (r.message) {
                EBE.State.posting_types = r.message.map(function (pt) { return pt.posting_type; });
            }
            $(page.body).html(EBE.UI_TEMPLATE());
            EBE.Events.setup(page);
        }
    });
};
